...
const EditPromotionArea = ({ open, handleClose, promocao, atualizarPromocao, produtos }) => {
    ...
    // Carregar dados da promoção quando o modal abrir ou a promoção mudar
    useEffect(() => {
        if (promocao) {
            setProdutoSelecionado(promocao.produto || "");
            
            // Extrair o valor numérico do desconto (remover o símbolo %)
            const descontoNumerico = promocao.desconto ? 
                promocao.desconto.replace(/[^\d]/g, '') : "";
            setDesconto(descontoNumerico);
            
            setStatus(promocao.status || "Ativo");
            setValorOriginal(promocao.valor || "");
            setValorComDesconto(promocao.valorComDesconto || "");
    ...
    const handleSubmit = () => {
        ...
        // Criar objeto com os dados atualizados da promoção
        ...
        // Chamar a função de atualizar promoção passada como prop
        atualizarPromocao(promocaoAtualizada);
    };
...
export default EditPromotionArea;
